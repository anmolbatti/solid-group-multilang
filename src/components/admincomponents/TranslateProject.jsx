...
function TranslateProject() {
    ...
    const fetchLanguages = async () => {
      try {
          ...
          if(response.data){
            const filteredLanguages = response.data.filter((language) => language.code !== "en");
        ...
        const fetchProjectDetails = async () => {
            try {
                ...
                setProject({
                  ...
                  galleryTitle1: response.data?.galleryTitle1,
                  galleryDesc1: response.data?.galleryDesc1,
                  galleryTitle2: response.data?.galleryTitle2,
                  galleryDesc2: response.data?.galleryDesc2,
                  isFeatured: response.data?.isFeatured,
                  isHomePageFeatured: response.data?.isHomePageFeatured,
                  isDetailPageFeatured: response.data?.isDetailPageFeatured,
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault();
        if(oldSlug.trim() === project?.slug.trim()){
          return alert("Project slug shouldn't be same");
        }
        ...
        formData.append('translatedSlug', project.slug);
        ...
        try {
            const response = await axios.post(`${config.BASE_URL}/api/admin/add-project`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
                withCredentials: true,
                ...
                <form onSubmit={handleFormSubmit} encType="multipart/form-data" className="add-post-form">
                    <div className="update_details_wrap">
                        ...
                        <div className="update_btn_wrap">
                            <button className="update_btn" type="submit">Translate Project</button>
                        </div>
